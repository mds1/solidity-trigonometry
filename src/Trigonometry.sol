...
library Trigonometry {
   ...
   * circle into a 30 bit angles, i.e. there are 1,073,741,824 (2^30) angle units, instead of the
   * standard 360 degrees or 2pi radians. From there, we get an output in range -2147483647 to 2147483647,
   * (which is the max value of an int32) which is then converted back to the standard range of -1 to 1,
   * again scaled by 1e18
   * @param _angle Angle to convert
   * @return Result scaled by 1e18
   */
  function sin(uint256 _angle) public pure returns (int256) {
    // Convert angle from from arbitrary radian value (range of 0 to 2pi) to the algorithm's range of 0 to 1073741824
    unchecked {
