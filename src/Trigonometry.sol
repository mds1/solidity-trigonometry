...
library Trigonometry {
  ...
  function convertAngle(uint256 _angle) internal pure returns (uint256) {
    // Convert angle from from radians (range of 0 to 2pi) to the algorithm's range of 0 to 16384
    uint256 _angleInRadians = _angle % (2 * PI); // wrap angle between 0 and 2pi
    return ANGLES_IN_CYCLE * _angleInRadians / 2 / PI;
  }
   ...
   * @return The sine result as a number in the range -32767 to 32767, which maps to a range of -1 to 1
   */
  function sin(uint256 _angle) public pure returns (int256) {
    _angle = convertAngle(_angle);
